...
def readCSV():
    ...
    # Graph the data
    jumping_pred = np.count_nonzero(prediction_results)
    walking_pred = len(prediction_results) - jumping_pred

    prediction = ('Walking', 'Jumping')
    values = (walking_pred, jumping_pred)

    if (walking_pred > jumping_pred):
        pred = 'walking'
    else:
        pred = 'jumping'

    fig = plt.figure(figsize=(5, 4))
    plt.bar(prediction, values, color='blue', width=0.4)
    plt.x_label = 'Prediction Options'
    ...
    label = tk.Label(window, text="Overall predicted motion is "+pred, width=40, height=5)
    label.pack()
    window.update()
    # Output CSV
